...
| # | 作業 | 担当 | 備考 |
|---|------|------|------|
| 0-1 | **説明会予約URLの設定** | 👤 | Googleフォーム / Calendly 等を用意し、`index.html` L934 の `href="#"` を差し替え |
| 0-2 | **GA4 プロパティ作成 + Measurement ID 取得** | 👤 | [analytics.google.com](https://analytics.google.com) でプロパティ作成 |
| 0-3 | **GA4 ID をサイトに反映** | 🔧 | `index.html` の `G-XXXXXXXXXX` を2箇所差し替え → デプロイ |
